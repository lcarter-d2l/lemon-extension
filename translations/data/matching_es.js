...
MatchingApp.buildApp = function() {
   ...
   MatchingApp.questionData = MatchingApp.AppData.PreguntaNodes;
   ...
   MatchingApp.AppData.DefaultColor = '#FFFFFF';
   MatchingApp.qCols = parseInt(MatchingApp.AppData.Advanced.PreguntaColumns);
...
MatchingApp.loadButtons = function() {
   ...
   MAT_checkLabel.innerHTML = 'Comprobar respuestas';
   ...
   MAT_restartLabel.innerHTML = 'Reiniciar actividad';
   ...
   MAT_continueLabel.innerHTML = 'Comenzar actividad';
   ...
   MAT_postQuizLabel.innerHTML = 'Continuar';
...
MatchingApp.buildReport = function() {
   ...
   /*resultsHeader.innerHTML = 'Comentarios:';*/
   var total = 0;
               ...
               switch (MatchingApp.scoreEval[j]) {
                  case 1:
                     ...
                     if (MatchingApp.questionData[j].correct !== 'none' && MatchingApp.questionData[j].correct !== null && MatchingApp.questionData[j].correct !== undefined && MatchingApp.questionData[j].correct !== '') {
                        tempFeedback.innerHTML = '<strong>Comentarios: </strong>' + MatchingApp.questionData[j].correct;
                  ...
                  default:
                     ...
                     if (MatchingApp.questionData[j].wrong !== 'none' && MatchingApp.questionData[j].wrong !== null && MatchingApp.questionData[j].wrong !== undefined && MatchingApp.questionData[j].wrong !== '') {
                        tempFeedback.innerHTML = '<strong>Comentarios: </strong>' + MatchingApp.questionData[j].wrong;
                        tempMiniReportContent.appendChild(tempFeedback);
                     }
 ...
 *
 * @method buildPreguntaCols
 */
MatchingApp.buildPreguntaCols = function() {
   ...
   for (c = 0; c < MatchingApp.AnsSlotData.length; c++) {
      if (MatchingApp.matchAmount[c] > 0) {
         ...
         $('<div/>', {
            ...
            title: MatchingApp.AnsSlotData[c].title + ' drop zone',
            style: style
         }).appendTo(tempContDiv);
...
MatchingApp.buildInstructions = function() {
   ...
   if (MatchingApp.mobileDevice === true) {
      if (MatchingApp.ActivityType === 'match') {
         if (MatchingApp.AppData.FeedbackType === 'report') {
            instructions = 'Match each item to the appropriate category. ' + 'Tap on "Comprobar respuestas" to see your results.';
         } else {
            instructions = 'Match each item to the appropriate category. ' + 'Tap on "Comprobar respuestas" to see your progress.';
         }
      } else {
         if (MatchingApp.AppData.FeedbackType === 'report') {
            instructions = 'Sort each item into the appropriate category. ' + 'Tap on "Comprobar respuestas" to see your results.';
         } else {
            instructions = 'Sort each item into to the appropriate category. ' + 'Tap on "Comprobar respuestas" to see your progress.';
         }
      }
   } else {
      if (MatchingApp.ActivityType === 'match') {
         if (MatchingApp.AppData.FeedbackType === 'report') {
            instructions = '<span class="MAT_instructionText">Clasifica cada elemento en la categoría adecuada arrastrando la opción en la zona para soltar (usuarios de computadoras) o seleccionando la opción y luego la zona para soltar (usuarios que usan pantallas más pequeñas o dispositivos móviles ). Los usuarios que solo usan el teclado deben usar la tecla de tabulación (tab) para seleccionar una opción de la lista y, luego, la tecla intro (enter) para seleccionarla. Presiona tab nuevamente para seleccionar la zona correcta donde soltar el elemento y presiona intro para colocarlo en la misma. Selecciona una opción y presiona la tecla suprimir para devolver el elemento a su posición original. Usa el botón "Comprobar respuestas" para obtener tus resultados.</span>';
         } else {
            instructions = '<span class="MAT_instructionText">Clasifica cada elemento en la categoría adecuada arrastrando la opción en la zona para soltar (usuarios de computadoras) o seleccionando la opción y luego la zona para soltar (usuarios que usan pantallas más pequeñas o dispositivos móviles ). Los usuarios que solo usan el teclado deben usar la tecla de tabulación (tab) para seleccionar una opción de la lista y, luego, la tecla intro (enter) para seleccionarla. Presiona tab nuevamente para seleccionar la zona correcta donde soltar el elemento y presiona intro para colocarlo en la misma. Selecciona una opción y presiona la tecla suprimir para devolver el elemento a su posición original. Usa el botón "Comprobar respuestas" para obtener tus resultados.</span>';
         }
      } else {
...
MatchingApp.reset = function() {
   ...
   $('#MAT_progress').html('Click the "Comprobar respuestas" button to check your progress.');
   ...
   if (MatchingApp.AppData.Randomize === true) {
      ...
      $(".MAT_qCol").each(function() {
         ...
         if (i == 0) return false;
         while (--i) {
            var j = Cálculo.floor(Cálculo.random() * (i + 1));
...
MatchingApp.parseData = function() {
   if (!MatchingApp.AppData.Advanced) {
      ...
      MatchingApp.AppData.Advanced.PreguntaColumns = MatchingApp.AppData.PreguntaColumns;
